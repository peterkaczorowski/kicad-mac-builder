...
if DYLD_PRINT_LIBRARIES=1 DYLD_PRINT_LIBRARIES_POST_LAUNCH=1 "$1" -c 'import wx ; print("Imported" + "Module" + "Successfully")' 2>&1 | grep /System/Library/Frameworks/Python.framework > /dev/null; then
...
if ! DYLD_PRINT_LIBRARIES=1 DYLD_PRINT_LIBRARIES_POST_LAUNCH=1 "$1" -c 'import wx ; print("Imported" + "Module" + "Successfully")' 2>&1 | grep ImportedModuleSuccessfully > /dev/null; then
